...
module pHModule
    ...
    # Exports for pH DAEs
    export pHDescriptorSystem, state_matrix, to_pHODE, is_index1_like, as_coenergy_state, check_coherence
    ...
    function pHSystem(J::AbstractMatrix{<:Real},
        ...
        Jf = _toT(J, T);  Rf = _toT(R, T);  Qf = _toT(Qe, T)
        ...
        return pHSystem{T, typeof(Jf), typeof(Rf), typeof(Qf), typeof(Gf)}(Jf, Rf, Qf, Gf, n, size(Gf,2))
    end
    ...
    function dynamics!(dx::AbstractVector, sys::pHSystem, x::AbstractVector, u::AbstractVector, Qx::AbstractVector)
    ...
    function pHDescriptorSystem(E::AbstractMatrix{<:Real},
        ...
        T = promote_type(Float64, eltype(E), eltype(J), eltype(R), eltype(Qe), eltype(G), (QH===nothing ? Float64 : eltype(QH)))
        E,J,R,Qe = (T.(E), T.(J), T.(R), T.(Qe))
        G = isa(G,AbstractVector) ? reshape(T.(G),:,1) : T.(G)
        n = size(E,1); m = size(G,2)
        ...
        return pHDescriptorSystem{T,typeof(E),typeof(J),typeof(R),typeof(Qe),typeof(G),typeof(QHn)}(
            E,J,R,Qe,G,QHn,n,m,rep
        )
    end
