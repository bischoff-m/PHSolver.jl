...
module utilsModule
    ...
    function blockmatrix(A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D::AbstractMatrix)
        mA, nA = size(A);  mB, nB = size(B)
        mC, nC = size(C);  mD, nD = size(D)
    ...
    function plot_states(t, X)
        tt = collect(t)
        n  = size(X, 1)
        p  = plot(tt, vec(@view X[1, :]), label = "x(1)", xlabel = L"Time $t$", ylabel = L"States $x$", title = "Simulation results")
        if n > 7
            for i in 2:7
    ...
    function plot_energy(t, X, sys)
        tt = collect(t)
        n  = size(X, 1)
        if sys isa Main.SystemModellingModule.pHModule.pHSystem
            Q  = getfield(sys, :Qe)
        elseif sys isa Main.SystemModellingModule.pHModule.pHDescriptorSystem
            Q = getfield(sys, :QH)
        else
    ...
    function plot_results(t, X; sys=nothing, H=nothing, separate::Bool=false, state_labels=nothing, energy_label::AbstractString=L"$\mathcal{H}$")
        ...
        if H === nothing && sys !== nothing
            ...
            elseif sys isa Main.SystemModellingModule.pHModule.pHSystem
        ...
        if separate
            ...
            p_energy = plot()
            if H !== nothing
                plot!(p_energy, t, H, l=:dash, label=energy_label, xlabel=L"Time $t$", title="Hamiltonian")
            else
